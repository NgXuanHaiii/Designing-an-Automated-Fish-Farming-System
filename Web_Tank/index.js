...
const firebaseConfig = {
    apiKey: "AIzaSyDgVHsv9iT27UbR5qOly58FslXgFtajT40",
    authDomain: "pj2-aqua.firebaseapp.com",
    databaseURL: "https://pj2-aqua-default-rtdb.firebaseio.com",
    projectId: "pj2-aqua",
    storageBucket: "pj2-aqua.appspot.com",
    messagingSenderId: "486810379158",
    appId: "1:486810379158:web:401f3d24e3b2d2e85233b7"
  };
...
firebase.database().ref("/pj2-aqua/anhsang").on("value",function(snapshot){
    const as = snapshot.val();  
    document.getElementById("anhsang").innerHTML = as;
    console.log(as);
});
...
var database = firebase.database();
    var camRef = database.ref("/pj2-aqua/cam");
    camRef.on("value", function(snapshot) {
        var camValue = snapshot.val();
        // Cập nhật giá trị cảm biến hồng ngoại lên trang web
        document.getElementById("cam-value").innerHTML = camValue === 0 ? "Có cám" : "Không có cám";
    });
    var database = firebase.database();
    var lightRef = database.ref("/pj2-aqua/light");
    lightRef.on("value", function(snapshot) {
        var lightValue = snapshot.val();
        // Cập nhật giá trị cảm biến hồng ngoại lên trang web
        document.getElementById("light-value").innerHTML = lightValue === 1 ? "Đèn đang bật" : "Đèn đang tắt";
    });



    firebase.database().ref("/pj2-aqua/sw_mode").on("value", function(snapshot) {
        const swModeValue = snapshot.val();
        toggles[0].classList.toggle('active', swModeValue === "ON");
    });
    
    firebase.database().ref("/pj2-aqua/sw_pump").on("value", function(snapshot) {
        const swPumpValue = snapshot.val();
        toggles[1].classList.toggle('active', swPumpValue === "ON");
    });
    
    firebase.database().ref("/pj2-aqua/sw_feed").on("value", function(snapshot) {
        const swFeedValue = snapshot.val();
        toggles[2].classList.toggle('active', swFeedValue === "ON");
    });
    
    firebase.database().ref("/pj2-aqua/light").on("value", function(snapshot) {
        const lightValue = snapshot.val();
        toggles[3].classList.toggle('active', lightValue === "ON");
    });
